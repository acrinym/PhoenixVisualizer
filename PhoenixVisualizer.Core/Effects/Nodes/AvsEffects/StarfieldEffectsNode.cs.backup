...
namespace PhoenixVisualizer.Core.Effects.Nodes.AvsEffects
{
    ...
    public class StarfieldEffectsNode : BaseEffectNode
    {
        ...
        public override void ApplyConfiguration(Dictionary<string, object> config)
        {
            ...
            if (config.TryGetValue("Speed", out var speed))
            ...
            if (config.TryGetValue("BeatSpeedBoost", out var beatBoost))
                BeatSpeedBoost = Convert.ToSingle(beatBoost);
            if (config.TryGetValue("DepthEffect", out var depthEffect))
                DepthEffect = Convert.ToBoolean(depthEffect);
            if (config.TryGetValue("MotionBlur", out var motionBlur))
                MotionBlur = Convert.ToBoolean(motionBlur);
            if (config.TryGetValue("Direction", out var direction))
