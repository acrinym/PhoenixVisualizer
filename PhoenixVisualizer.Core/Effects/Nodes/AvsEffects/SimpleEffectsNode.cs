...
namespace PhoenixVisualizer.Core.Effects.Nodes.AvsEffects
{
    ...
    public class SimpleEffectsNode : BaseEffectNode
    {
        ...
        public override void ProcessFrame(Dictionary<string, object> inputData, Dictionary<string, object> outputData, AudioFeatures audioFeatures)
        {
            ...
            try
            {
                var canvas = GetInputValue<ImageBuffer>("Canvas", inputData);
                if (canvas?.Data == null) return;

                var outputCanvas = new ImageBuffer(canvas.Width, canvas.Height);
                Array.Copy(canvas.Data, outputCanvas.Data, canvas.Data.Length);

                // Get audio data based on channel mode
                float[] audioData = GetAudioData(audioFeatures);
                if (audioData == null) return;

                // Process based on visualization mode
                if (VisualizationMode <= 2) // Spectrum modes
                {
                    ProcessSpectrumVisualization(outputCanvas, audioData);
                }
                else // Oscilloscope modes
                {
                    ProcessOscilloscopeVisualization(outputCanvas, audioData);
                }

                outputData["Output"] = outputCanvas;
            }
            catch (Exception ex)
            {
                System.Diagnostics.Debug.WriteLine($"[Simple Effects] Error processing frame: {ex.Message}");
            }
        }
        ...
        public override void ApplyConfiguration(Dictionary<string, object> config)
        {
            ...
            if (config.TryGetValue("VisualizationMode", out var mode))
                VisualizationMode = Convert.ToInt32(mode);
            
            if (config.TryGetValue("ChannelMode", out var channel))
                ChannelMode = Convert.ToInt32(channel);
            
            if (config.TryGetValue("Color", out var color))
                Color = (Color)color;
            
            if (config.TryGetValue("XPosition", out var xPos))
                XPosition = Convert.ToInt32(xPos);
            
            if (config.TryGetValue("YPosition", out var yPos))
                YPosition = Convert.ToInt32(yPos);
            
            if (config.TryGetValue("VisualizationWidth", out var width))
                VisualizationWidth = Convert.ToInt32(width);
            
            if (config.TryGetValue("VisualizationHeight", out var height))
                VisualizationHeight = Convert.ToInt32(height);
            
            if (config.TryGetValue("TriggerLevel", out var trigger))
                TriggerLevel = Convert.ToSingle(trigger);
            
            if (config.TryGetValue("SpectrumBands", out var bands))
                SpectrumBands = Convert.ToInt32(bands);
            
            if (config.TryGetValue("PeakHoldTime", out var peakTime))
                PeakHoldTime = Convert.ToInt32(peakTime);
            
            if (config.TryGetValue("DrawPeaks", out var drawPeaks))
                DrawPeaks = Convert.ToBoolean(drawPeaks);
            
            if (config.TryGetValue("Smoothing", out var smoothing))
                Smoothing = Convert.ToSingle(smoothing);
        }
