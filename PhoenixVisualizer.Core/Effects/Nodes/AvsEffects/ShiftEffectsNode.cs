    ...
    public class ShiftEffectsNode : BaseEffectNode
    {
        ...
        public override void ProcessFrame(Dictionary<string, object> inputData, Dictionary<string, object> outputData, AudioFeatures audioFeatures)
        {
            ...
            try
            {
                ...
                ApplyShiftTransformation(sourceImage, outputImage);

                outputData["Output"] = outputImage;
            }
            catch (Exception ex)
            {
                System.Diagnostics.Debug.WriteLine($"[Shift Effects] Error processing frame: {ex.Message}");
            }
        }
        ...
        public override Dictionary<string, object> GetConfiguration()
        {
            return new Dictionary<string, object>
            {
                ...
                { "BeatDisplacementX", BeatDisplacementX },
                { "BeatDisplacementY", BeatDisplacementY },
                { "EdgeMode", EdgeMode },
        ...
        public override void ApplyConfiguration(Dictionary<string, object> config)
        {
            ...
            if (config.TryGetValue("DisplacementX", out var displacementX))
                DisplacementX = Convert.ToSingle(displacementX);
            
            if (config.TryGetValue("DisplacementY", out var displacementY))
                DisplacementY = Convert.ToSingle(displacementY);
            
            if (config.TryGetValue("BlendingMode", out var blendingMode))
                BlendingMode = Convert.ToInt32(blendingMode);
            
            if (config.TryGetValue("SubpixelPrecision", out var subpixel))
                SubpixelPrecision = Convert.ToBoolean(subpixel);
            
            if (config.TryGetValue("BilinearInterpolation", out var bilinear))
                BilinearInterpolation = Convert.ToBoolean(bilinear);
            
            if (config.TryGetValue("BeatReactive", out var beatReactive))
                BeatReactive = Convert.ToBoolean(beatReactive);
            
            if (config.TryGetValue("BeatMultiplier", out var beatMultiplier))
                BeatMultiplier = Convert.ToSingle(beatMultiplier);
            
            if (config.TryGetValue("BeatDisplacementX", out var beatDispX))
                BeatDisplacementX = Convert.ToSingle(beatDispX);
            
            if (config.TryGetValue("BeatDisplacementY", out var beatDispY))
                BeatDisplacementY = Convert.ToSingle(beatDispY);
            
            if (config.TryGetValue("EdgeMode", out var edgeMode))
                EdgeMode = Convert.ToInt32(edgeMode);
            
            if (config.TryGetValue("DisplacementMode", out var dispMode))
                DisplacementMode = Convert.ToInt32(dispMode);
            
            if (config.TryGetValue("MovementSpeed", out var speed))
                MovementSpeed = Convert.ToSingle(speed);
            
            if (config.TryGetValue("AudioSensitivity", out var sensitivity))
                AudioSensitivity = Convert.ToSingle(sensitivity);
        }
