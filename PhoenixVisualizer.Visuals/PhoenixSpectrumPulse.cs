...
public sealed class PhoenixSpectrumPulse : IVisualizerPlugin
{
    ...
    private uint CalculateBarColor(float frequencyRatio, float magnitude, float volume, int barIndex, AudioFeatures f)
    {
        ...
        // Convert HSV to RGB
        return PhoenixVisualizer.Core.ColorUtils.ColorUtil.HsvToRgb(hue, saturation, brightness);
    }
    ...
    private uint PhoenixVisualizer.Core.ColorUtils.ColorUtil.HsvToRgb(float hue, float saturation, float brightness)
    {
